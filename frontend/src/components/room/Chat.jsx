...
const Chat = ({ socket, roomId, isOpen, onToggle, onResize }) => {
  ...
  const [chatWidth, setChatWidth] = useState(320); // 320px = 80 in tailwind (w-80)
  ...
  const MIN_WIDTH_PERCENT = 0.10; // 10%
  const MAX_WIDTH_PERCENT = 0.35; // 35%
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      // Show scrollbar only when content exceeds 240px
  ...
  useEffect(() => {
    ...
    // Listen for authenticated user info from server
    socket.on('authenticated-user', (data) => {
      console.log('Authenticated user data received:', data);
    ...
    return () => {
      ...
      socket.off('authenticated-user');
    };
  }, [socket]);

  // Auto-set username for authenticated users
  useEffect(() => {
    if (isAuthenticated && user && user.username && !isUsernameSet) {
      setUsername(user.username);
      setIsUsernameSet(true);
      console.log('Auto-set username from auth:', user.username);
    }
  }, [isAuthenticated, user, isUsernameSet]);
  ...
  const handleSetUsername = (e) => {
    ...
    // Prevent authenticated users from manually setting username
    if (isAuthenticated && user) {
      setUsernameError('You are already logged in as ' + user.username);
      return;
    }
  ...
  const handleKeyDown = (e) => {
    // Allow Shift+Enter for new line, prevent Enter alone from submitting
  ...
  return (
    ...
    >
      ...
      {/* Username Setup - Only show for non-authenticated users */}
      {!isUsernameSet ? (
        <div className="flex-1 flex items-center justify-center p-6">
          <form onSubmit={handleSetUsername} className="w-full">
            <label className="block text-gray-700 font-semibold mb-2">
              {isAuthenticated ? 'Setting up your chat...' : 'Enter your name to start chatting:'}
            </label>
            {!isAuthenticated && (
              <>
                <input
                ...
                <div key={index}>
                  {msg.type === 'system' ? (
                    ...
                    <div className={`flex flex-col ${msg.isOwn ? 'items-end' : 'items-start'}`}>
                      <div className={`max-w-[75%] rounded-lg px-3 py-2 ${msg.isOwn
                        ? 'bg-blue-600 text-white'
                        : 'bg-white border border-blue-200'
                        }`}>
                        <p className={`text-xs font-semibold mb-1 ${msg.isOwn ? 'text-blue-100' : 'text-blue-600'
                          }`}>
                          {msg.username}
                        </p>
